...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "0 leaves its default (one thread per processor). A scheduling knob: it "
        "changes neither the circuit nor the analysis, only how ngspice schedules "
        "the solve. On a contended host, 3.4x faster wall / 16x less CPU at N=1 for "
        "a ~1300-device deck, because the default makes concurrent -j points fight "
        "for cores and spin. Grid throughput comes from -j; this stops each point "
        "from oversubscribing the machine. MEASURED VALUES are unaffected on every "
        "deck A/B'd so far, including a full-length gate-level one: identical to all "
        "ten printed digits at N=1 vs N=4 on sim/sar-logic-timing (ideal XSPICE), "
        "sim/cdac-bit-settling (real PDK devices) and all 16 measurements of "
        "sim/sar-logic-functional-gates (synthesized standard cells, full 64.5 us "
        "run). The ACCEPTED-TIMESTEP SEQUENCE is a separate claim and is NOT "
        "invariant: on the two small decks the timepoint counts match exactly, but "
        "on gate-level decks N perturbs the sequence from the 2nd accepted point "
        "(11590 vs 11538 points on sar-logic-functional-gates; 2039 vs 4311 on "
        "sar-logic-timing-gates). Both are valid solutions within the solver's "
        "tolerance -- see sim/sar-logic-functional-gates/investigations/"
        "20260918-issue-309-flag-measurement-neutrality.md. Note also that under a "
        "tight CPU share N>1 buys nothing: N=4 accrues the same total CPU as N=1, "
        "split four ways.",
    )
    parser.add_argument(
        ...
        "MEASURED VALUES are unaffected on every deck A/B'd so far, including a "
        "full-length gate-level one: identical to all ten printed digits on "
        "sim/sar-logic-timing (ideal XSPICE), sim/cdac-bit-settling (real PDK "
        "devices) and all 16 measurements of sim/sar-logic-functional-gates "
        "(synthesized standard cells, full 64.5 us run, zero 'out of interval' "
        "meas lines). The ACCEPTED-TIMESTEP SEQUENCE is a separate claim and is NOT "
        "invariant: on the two small decks the timepoint counts match exactly, but "
        "on gate-level decks the save list perturbs the sequence from the 3rd "
        "accepted point (11589 vs 11590 points on sar-logic-functional-gates; 2574 "
        "vs 2039 to the same 250 ns on sar-logic-timing-gates). Both are valid "
        "solutions within the solver's tolerance, so compare two records "
        "digit-for-digit only at matching retention AND thread settings -- see "
        "sim/sar-logic-functional-gates/investigations/"
        "20260918-issue-309-flag-measurement-neutrality.md. Refuses (exit 3) on a "
