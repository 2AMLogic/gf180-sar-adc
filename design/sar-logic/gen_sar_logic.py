...
def _loop(
    ...
    `dut_subckt` names the subckt this loop instantiates in place of
    `sar_ctrl_a` -- the rung-1 ideal-XSPICE model by default, unchanged, so
    every existing caller (and the byte-identical committed netlists they
    produce) is untouched. issue #273's gate-level replay passes the
    translated gate-netlist subckt's own name instead (both wrap the exact
    same 71-net port list, so no other line in this function needs to change
    -- see `design/sar-logic/rtl/README.md`'s port-compatibility note).

    `start_pulse_clocks` is the rung-1 default (0): the ideal model seeds its
    ring with `ph15`'s own `ic=1`, so the two ideal decks never need to drive
    `start` at all (`dc 0`, unchanged). A gate-level DUT has no such seed --
    a real standard-cell flip-flop carries no reset pin at all -- so
    `design/sar-logic/rtl/README.md`'s "Real hardware note" requires `start`
    to be asserted for at least one clock after power-up before the ring's
    one-hot invariant can be trusted. A positive value here drives `start`
    high from t=0 for that many clock periods (a `pwl` ramp, not an ideal
    ...
    ports[3] = f"{tag}_cmpo"
    a(f"* ---- loop {tag} ----")
    if start_pulse_clocks > 0:
        pw_ns = start_pulse_clocks * CLK_PERIOD_NS
        a(
            f"v{tag}start {tag}_start 0 pwl(0 {{vdd_val}} {pw_ns:g}n {{vdd_val}} "
            f"{pw_ns + 0.1:g}n 0)"
        )
    else:
        a(f"v{tag}start {tag}_start 0 dc 0")
    a(f"v{tag}mode {tag}_mode 0 dc {mode_expr}")
    L += _wrap(f"x{tag}", ports + [dut_subckt])
...
def _functional_body(
    ...
    *,
    ...
    a("*")
    a(f"* GENERATED by {generator_path} -- do not edit.")
    a("*")
...
#: The five loops `_timing_body` can instantiate, in emission order. Also
#: the canonical `loop_tags` value -- passing this whole tuple (the default)
#: reproduces the original one-deck composition byte-for-byte; issue #311's
#: per-loop gate-level decks (`gen_sar_ctrl_gates_tb.py`) pass a single-tag
#: subset instead.
TIMING_LOOP_TAGS: tuple[str, ...] = ("ok", "lt", "xl", "bad", "tie")

#: `cmp_delay` for each non-`tie` timing loop -- `None` means "no delay
#: insertion" (`ok`'s reference case), forwarded to `_loop` unchanged.
_TIMING_DELAY_BY_TAG: dict[str, str | None] = {
    "ok": None, "lt": "40n", "xl": "50n", "bad": "70n",
}


def _timing_header(loop_tags: tuple[str, ...], generator_path: str) -> list[str]:
    """The `tb_sar_logic_timing`-family header comment.

    Byte-identical to the ORIGINAL five-loop header when `loop_tags` is
    exactly `TIMING_LOOP_TAGS` in that order (`timing()`/the gate-level
    five-loop deck, both unaffected by issue #311). Any proper subset gets a
    shorter, per-tag header instead: the full text above assumes all five
    loops share the deck it is written into -- e.g. `bad`'s negative-control
    paragraph explicitly says "it is the reason the other three mean
    anything" -- which stops being literally true once a single loop is the
    whole deck.
    """
    L: list[str] = []
    a = L.append
    ...
    a(f"* GENERATED by {generator_path} -- do not edit.")
    a("*")
    if tuple(loop_tags) == TIMING_LOOP_TAGS:
        ...
        a("* ISSUE #311: this deck instantiates ONE loop of the five-loop")
        a("* family above (ok/lt/xl/bad/tie), not all five. ngspice advances")
        a("* one GLOBAL timestep for a whole deck, so five clock-sharing")
        a("* sar_ctrl_a instances sharing one file force each other's step")
        a("* down to the union of their switching activity -- measured at up")
        a("* to ~100x the cost per simulated ns of running one loop alone")
        a("* (sim/sar-logic-timing-gates/investigations/")
        a("* 20260917-issue-303-five-loop-composition-cost.md). The loops")
        a("* are electrically independent -- each has its own sar_ctrl_a")
        a("* instance, CDAC/comparator and stimulus, sharing only the `clk`")
        a("* stimulus and the `vdd_gate` supply -- so splitting them changes")
        a("* NOTHING about what is measured, only how many loops share one")
        a("* deck. The measurement definitions, bounds and 8.5 us duration")
        a("* are UNCHANGED from the five-loop composition.")
        a("*")
        for tag in loop_tags:
            if tag == "ok":
                a("* `ok`: 0 ns delay -- the reference point. No delay is")
                a("* inserted between the comparator's analog decision and")
                a("* the controller sampling it, so with no added decision")
                a("* delay every conversion must come out exact.")
            elif tag == "lt":
                a("* `lt`: 40 ns delay -- the whole of the ~40 ns of")
                a("* per-cycle slack the survey (Sec 4.2) claims a 62.5 ns")
                a("* bit cycle has, spent in one place. The conversion must")
                a("* still come out exact.")
            elif tag == "xl":
                a("* `xl`: 50 ns delay -- 80% of the bit cycle consumed,")
                a("* still correct. Not the survey's original guess: an")
                a("* earlier draft of the five-loop deck used 55 ns and")
                a("* measured a real failure (max|err| = 2 LSB against a")
                a("* >= 1 LSB threshold). Bisecting against the same")
                a("* behavioural DAC/comparator model found the true")
                a("* boundary between 50 ns (exact) and 52 ns (max|err| =")
                a("* 1 LSB, already over the 0.5 LSB bound). 50 ns is kept")
                a("* as the bracket point precisely because it is the")
                a("* largest round number this model measures as exact,")
                a("* not because it was the original guess -- CLAUDE.md")
                a("* requires a measured number here, not an assumed one.")
                a("* DR-0010 records the gap between the survey's slack")
                a("* estimate and this measured boundary; the shortfall is")
                a("* attributed to this testbench's RC DAC-settling model")
                a("* (tau = 2.56 ns) consuming part of the delay budget the")
                a("* survey treated as separate, not to a synchronous-logic")
                a("* problem.")
            elif tag == "bad":
                a("* `bad`: 70 ns delay -- DELIBERATELY past the 62.5 ns")
                a("* cycle: the decision arrives after the edge that was")
                a("* supposed to latch it. This is the family's NEGATIVE")
                a("* CONTROL -- its check asserts the conversion IS wrong.")
                a("* Run alone it is not itself a cross-check that")
                a("* `ok`/`lt`/`xl` pass for the right reason (that")
                a("* cross-check needs all four sharing one run, which the")
                a("* five-loop deck under sim/sar-logic-timing-gates/ still")
                a("* exists to provide at nominal PVT); what it does still")
                a("* prove, alone, is that this exact testbench methodology")
                a("* is capable of detecting a broken conversion, on this")
                a("* DUT, at this corner.")
            else:  # tie
                a("* `tie`: input pinned exactly on the free-MSB decision")
                a("* threshold (v_in = V_cm) for the whole run -- the")
                a("* near-metastable input a real comparator resolves")
                a("* slowly or wrongly. The claim under test is NOT that")
                a("* the code is right (on an exact tie either adjacent")
                a("* code is correct) but that the conversion still")
                a("* COMPLETES on schedule: drdy keeps arriving exactly 16")
                a("* clocks apart. That is the concrete form of the row")
                a("* spec/prior-art-survey.md Sec 4.3 gives synchronous")
                a("* logic ('a stalled decision corrupts one bit') against")
                a("* asynchronous ('a stalled decision stalls the whole")
                a("* conversion; needs a timeout/watchdog').")
            a("*")
    a("* ==================================================================")
    return L
...
def _timing_body(
    *,
    ...
    generator_path: str = "design/sar-logic/gen_sar_logic.py",
    cmp_out_rc: tuple[str, str] | None = None,
    loop_tags: tuple[str, ...] = TIMING_LOOP_TAGS,
) -> list[str]:
    ...
    `loop_tags` selects which of the five loops (`TIMING_LOOP_TAGS`: `ok`/
    `lt`/`xl`/`bad`/`tie`) this deck instantiates. The default is all five,
    in their original order, which reproduces the ORIGINAL five-loop deck
    byte-for-byte -- `timing()` below, and the gate-level five-loop target
    in `gen_sar_ctrl_gates_tb.py`, both pass no argument here and are
    therefore unaffected. Issue #311: ngspice advances one GLOBAL timestep
    per deck, so five clock-sharing `sar_ctrl_a` instances sharing one file
    force each other's step down to the union of their own switching
    activity -- measured at up to ~100x the cost per simulated ns of running
    one loop alone (`sim/sar-logic-timing-gates/investigations/
    20260917-issue-303-five-loop-composition-cost.md`). The loops are
    electrically independent (each owns its own DUT instance, CDAC/
    comparator and stimulus, sharing only the `clk` stimulus and the
    `vdd_gate` supply), so a non-default `loop_tags` changes nothing about
    what is measured -- only how many loops share this deck.
    """
    if not loop_tags:
        raise ValueError("loop_tags must be non-empty")
    unknown = [t for t in loop_tags if t not in TIMING_LOOP_TAGS]
    if unknown:
        raise ValueError(
            f"loop_tags contains unknown tag(s) {unknown!r}; "
            f"must be a subset of {TIMING_LOOP_TAGS!r}"
        )
    L: list[str] = []
    a = L.append
    L += _timing_header(tuple(loop_tags), generator_path)
    a("")
    a(".param vref={vdd_val}")
    a(".param vcm={vdd_val/2}")
    a(".param vth={vdd_val/2}")
    ...
    for tag in ("ok", "lt", "xl", "bad"):
        if tag not in loop_tags:
            continue
        delay = _TIMING_DELAY_BY_TAG[tag]
        L += _loop(
