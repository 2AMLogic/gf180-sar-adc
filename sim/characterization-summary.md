...
**The currency gap this pass declared is now closed** by the #389 pass below,
which reconciled the `Supply` row against PR #387's rewrite of
`README.md#target-specification`. It is no longer a gap this document carries.

### Partial update — 2026-09-23, issue #358 (NOT a new full re-read)

**The full re-read above still stands at `93ddfe3` (2026-09-21); like the
#381 pass above, this is a narrower, declared update on top of it.** Issue
#358 — DR-0026's named follow-up — re-ran the ratified decks against a real
external `V_cm` drive network at DR-0026's derived budget, over each deck's
own governing PVT grid, as paired same-commit arms. What this pass touched,
and only this:

- **The `V_CM` row** now carries a measured full-PVT result against the
  ratified rows, replacing the one-temperature / one-supply exploratory
  sensitivity sweep it previously cited. Findings, method, and what was not
  run: [`sim/vcm-full-pvt/README.md`](vcm-full-pvt/README.md).
- **The follow-up list above** that named #358 as an open gap.
- **No verdict moves**, no row was added or removed, no other row's value or
  citation was touched, and no bound, target or manifest check was changed.
- `spec/testbench-suite-memo.md` §12 item 3 was rewritten by the same pass, so
  the Freshness table's row for that document above (which still describes
  PR #262's version of §12 item 3, citing the ≈ 0.2 LSB exploratory figure)
  should be read against the new §12 item 3 rather than the old one.

**Gaps this pass declares rather than closes**: the campaign's **extracted**
arms ran on the pre-#381 extraction vintage, which the #381 pass above
superseded while #358 was in review — so the `V_cm` delta has *not* been
re-taken on the current governing extraction (filed as **#392**, and the
direction of the re-extraction's move makes the committed headroom figure the
pessimistic one). `ENOB`/`SFDR` and the extracted power deck were not re-run
against the network at all.

### Partial update — 2026-09-23, issue #389 (NOT a new full re-read)

**The full re-read above still stands at `93ddfe3` (2026-09-21); like the two
passes above, this is a narrower, declared update on top of it.** PR #387
(issue #379) rewrote `README.md#target-specification`'s **Supply** row after
that re-read, to carry [DR-0036](../spec/decision-records/DR-0036-vdd-decoupling-budget.md)'s
external decoupling figure (`≥ 40 nF` at the `V_DD` pin), its source-impedance
budget (`≤ 3 Ω` in the switching band, `Z_vdd ≈ ≤ 1.6 Ω @ 2 MS/s` at the
stretch rate) and a required-but-unsized on-die local decoupling term inside
`ADC_BLOCK`. This pass reconciles this document's `Supply` row with that row.
What it touched, and only this:

- **The `Supply` row** now states all three clauses of the post-#387 ratified
  row, reports the ±10 % window and the DR-0036 budget as two separately
  verified halves, and cites
  [`sim/adc-rail-current/`](adc-rail-current/records/20260923-002945-1cefe83.md)
  for the 34.383980 mA CDAC switching peak (and the 39.5421 / 40.120 µA
  averages) the budget is derived from.
- **The "not yet measured" list** gained the unsized on-die `ADC_BLOCK`
  decoupling term, which is a required deliverable with no value to put in a
  row.
- **The Freshness gap #381 declared** — this exact reconciliation — is removed
  above, because this pass closes it.
- **No verdict moves, no other row was touched, and no bound, target or
  manifest check was changed.** No figure in this pass is new: every number
  is transcribed from DR-0036, from `sim/adc-rail-current/`'s committed record
  or from `layout/power/`'s, exactly as this document's own "creates no
  numbers of its own" contract requires.

**DR-0036 is *proposed*, not operator-ratified.** Its own status line reads
"proposed — requires operator sign-off", and `README.md`'s Supply row embeds
that caveat inline. The `Supply` row below therefore reports **against** the
proposed budget while saying it is unratified — the same footing on which the
`Area` row reports DR-0024's proposed `< 0.16 mm²`. Nothing here should be read
as operator sign-off having happened.

**Gaps this pass declares rather than closes**: no deck drives `V_DD` through
a real source network at the budget (every deck models the supply islands as
ideal DC sources), so the budget has **no measured verdict** — filed as
**#393**, the `V_CM` row's #358 done for this rail; the switching event's
charge is bounded from a window average rather than integrated (**#386**); and
DR-0036 step 8's 523 Ω effective resistance was read off the **pre**-Metal2-strap
`layout/power/` solve, which #378 / PR #388 superseded *after* DR-0036 merged
— DR-0036's own Consequences require whichever of the two is settled second to
re-read the other, and that re-read has not happened.

### Partial update — 2026-09-24, issue #395 (NOT a new full re-read)

**The full re-read above still stands at `93ddfe3` (2026-09-21); like the
three passes above, this is a narrower, declared update on top of it.** The
`V_CM` row's #358 campaign published its power arm with the sign of `i(vcms)`
reversed: the generated deck's V_cm ammeter (`sim/vcm-drive-impedance/gen_vcm_variant.py`)
was wired positive-terminal-to-the-source, the reverse of the ideal source
line it substitutes for, so the `p_vcm` term folded into `p_total` carried the
wrong sign in the V_cm arm and in **only** that arm. What this pass touched,
and only this:

- **The `V_CM` row's Power sentence.** `worst p_total 207.884 → 223.961 µW,
  margin 4.81× → 4.47×` is withdrawn and replaced by the corrected re-run,
  `207.884 → 203.608 µW, margin 4.81× → 4.91×`. The direction of the effect
  reverses: at DR-0026's budget the network leaves worst-case total power
  marginally *lower* than the ideal source does, because `C_dec` supplies the
  switching transients the pin would otherwise deliver.
- **Nothing else.** No other row, value, verdict, citation, bound, target or
  manifest check was touched. The `Power @ 1 MS/s` row itself is **not**
  affected: it cites the extracted governing record, which was never part of
  the V_cm campaign, and the campaign's own ideal-source control (207.884 µW)
  carries no ammeter and is unchanged.

**No verdict moves.** `p_vcm_*` carries no bound in
`sim/adc-power/testbench/tb.json`, and every `p_total_*` stayed an order of
magnitude inside the 1 mW check under both the wrong and the right sign, so
all 27 points read PASS either way. The error was a wrong *number* in a
passing row, not a missed failure.

Evidence: the superseded record is
[`20260923-114021-836a876`](adc-power/records/20260923-114021-836a876.md)
(kept, per this repo's append-only `sim/` convention); the corrected one is
[`20260924-041435-cda7de5`](adc-power/records/20260924-041435-cda7de5.md),
which names it in its own `Supersedes` field. Mechanism, the corrected
paired table, and the reproduction fidelity of the unchanged columns:
[`sim/vcm-full-pvt/README.md`](vcm-full-pvt/README.md) §"The power arm's sign
error".

**Gaps this pass declares rather than closes**: all three gaps the #358 entry
declared are untouched and still open (**#392**, and `ENOB`/`SFDR` plus the
...
is cited from. **No ratified row moves outside its bound** — and the
worst-case margin that statement rests on is now a measured number rather than
an untested assumption: **0.2126 LSB** of remaining headroom against the
ratified `< 1 LSB` window (`dnl_t767_t768_lsb`, `ss_125c_2.97v`), which is a
materially stronger statement than the one this list recorded as missing.
Three gaps are carried forward rather than closed: two decks named there are
still not re-run (`ENOB`/`SFDR`, and the extracted power netlist), both wired
into `sim/vcm-full-pvt/run_full_pvt.sh` with the reasons recorded in
[`sim/vcm-full-pvt/README.md`](vcm-full-pvt/README.md); and that campaign's
**extracted** arms were taken on the pre-#381 extraction vintage, which
issue #381 superseded while #358 was in review, so the V_cm delta has not been
re-taken on the current governing extraction (**#392**, and the same README's
"Which extraction this ran on").
...
| V_CM (Z_vcm, C_dec) | V_cm = V_REF/2 = 1.65 V ext.; ≥ 40 nF decoupling; Z_vcm ≤ 220 Ω ([DR-0026](../spec/decision-records/DR-0026-vcm-drive-source.md), **proposed — not yet operator-ratified**) | **The budget is derived; the converter's sensitivity to a real V_cm network at that budget is now measured against the ratified rows themselves, at full PVT, on the governing netlists (issue #358).** Replacing the ideal, zero-impedance V_cm source every other ADC-level deck in this table uses with an R‖L + C_dec network at DR-0026's budget (Z_vcm = 220 Ω, C_dec = 40 nF, modelled exactly the way these decks already model V_REF, DR-0002) was run as **paired same-commit arms** — each deck taken twice, once with its ideal source and once with the network, so the difference carries the network and nothing else. Eight arms, every point PASS, every arm clean-tree; each ideal control arm reproduces the committed citation it controlled for **at the commit the campaign ran on** (extracted INL/DNL 0.528287 / 0.727556 LSB, i.e. the pre-#381 `…076d545` citation — see the vintage caveat below; schematic 0.1100 / 0.0938 LSB; Power 207.884 µW — the same numbers, transitions and corners, the latter two still current). **On that extracted netlist, worst INL moves 0.5283 → 0.6651 LSB and worst DNL 0.7276 → 0.7874 LSB.** Two distinct figures, stated separately because they are different quantities: the **worst-case remaining margin** under the budget — the number DR-0026's sign-off actually rests on — is **0.2126 LSB** against the ratified `< 1 LSB` window (`dnl_t767_t768_lsb`, `ss_125c_2.97v`, where the ideal control arm already sits at 0.2724 LSB, so the network costs 0.0599 LSB there); the **largest single paired move** is **0.6163 LSB** (`dnl_t1_t2_lsb`, `ss_27c_2.97v`), which at that corner leaves 0.3334 LSB — 0.54× the move, i.e. 65 % of the margin that transition had. Points outside the < 0.5 LSB **stretch** target go from **1 of 27 to 20 of 27**. **Extraction-vintage caveat, flagged not absorbed**: the extracted pair ran on the pre-#381 netlist; issue #381 re-extracted and moved the ideal arm's worst DNL to 0.681240 LSB (minimum headroom 0.3188 LSB) — *away* from the bound — so the 0.2126 LSB figure is the pessimistic one and the V_cm delta on the current extraction is unmeasured (**#392**). The schematic, Power and sampling arms are unaffected by that re-extraction. Schematic side: worst INL 0.1100 → 0.4834 LSB, worst DNL 0.0938 → 0.4856 LSB (63/63 points). Power: worst p_total 207.884 → **203.608 µW**, margin 4.81× → **4.91×** against the 1 mW target — i.e. the network very slightly *reduces* worst-case total power, because `C_dec` supplies the switching transients the ideal source otherwise delivers through the pin. **This corrects the 207.884 → 223.961 µW / 4.47× figure this row carried until issue #395**: the V_cm arm's ammeter had been generated with its terminals reversed relative to the ideal source line it substitutes for, which flipped the sign of `i(vcms)` and therefore of the `p_vcm` term inside `p_total`; the superseding record is [`20260924-041435-cda7de5`](adc-power/records/20260924-041435-cda7de5.md) (superseding `20260923-114021-836a876`). The paired ideal-source control (207.884 µW) is unaffected — its deck carries no ammeter — and was re-run point-for-point at the corrected commit, reproducing every measurement to the last printed digit. The row's PASS verdict never depended on the error. `sim/dr0014-sampling/` is **insensitive** by more than three orders of magnitude (`samp_gain_err_lsb` 12.7674 → 12.7676 LSB), which is the mechanism DR-0026 derives showing up as data: that deck holds V_cm through one sampling event, while the converter deck releases the array onto V_cm twice per conversion. **The ≈ 0.2 LSB the exploratory sweep published is reproduced exactly at its own point** (`tt_27c_3.30v`: `gain_err_lsb` −2.00532 → −2.20527, `inl_t256_lsb` −0.01342 → −0.28053) and understates the effect ≈ 3× because of the transitions it quoted and its capacitor-only process axis — **not** because it held temperature and supply fixed (the worst move varies only 0.3765–0.4669 LSB across the whole 63-point grid). ENOB/SFDR and the extracted power deck were **not** re-run (both wired into `sim/vcm-full-pvt/run_full_pvt.sh`); see that README's findings for the margin arithmetic that deferred them | **PASS — no ratified row moves outside its bound under a real V_cm network meeting DR-0026's budget, at full PVT, on the schematic and on the extracted netlist the INL/DNL row was cited from when the campaign ran.** This is now a measured result against the ratified rows, not a stated gap: the ideal-source assumption is **not conservative**, and the price is quantified — the INL/DNL row's worst-case remaining margin under the budget is **0.2126 LSB** of its `< 1 LSB` window, and at the corner the network perturbs hardest it keeps only 0.54× of the perturbation (0.3334 LSB after a 0.6163 LSB move). **Flagged, not absorbed**: no bound, target or manifest was changed by this campaign; the extracted arms predate #381's re-extraction (**#392**); and ENOB/SFDR and the extracted power deck were not re-run at all. DR-0026 is still *proposed*, pending operator ratification, and the sign-off question it now carries is whether 220 Ω is the right ceiling given how much of that margin it spends | **Full-PVT paired campaign (issue #358)**: [`sim/adc-inl-dnl/records/20260923-095803-836a876.md`](adc-inl-dnl/records/20260923-095803-836a876.md) (**extracted, governing** — ideal-V_cm control) and [`20260923-100947-836a876.md`](adc-inl-dnl/records/20260923-100947-836a876.md) (extracted, V_cm network at budget); [`20260923-072117-664c8dc.md`](adc-inl-dnl/records/20260923-072117-664c8dc.md) / [`20260923-071835-664c8dc.md`](adc-inl-dnl/records/20260923-071835-664c8dc.md) (schematic, 63 pt, the pair); [`sim/adc-power/records/20260923-112459-836a876.md`](adc-power/records/20260923-112459-836a876.md) / [`20260923-114021-836a876.md`](adc-power/records/20260923-114021-836a876.md) (Power, the pair); [`sim/dr0014-sampling/records/20260923-085243-664c8dc.md`](dr0014-sampling/records/20260923-085243-664c8dc.md) / [`20260923-092849-664c8dc.md`](dr0014-sampling/records/20260923-092849-664c8dc.md) (sampling, the pair); [`sim/vcm-full-pvt/README.md`](vcm-full-pvt/README.md) (findings, method, and what was not run). **Prior exploratory sweep (issue #260, superseded as the citation for this row but retained):** [`sim/vcm-drive-impedance/records/20260825-162620-e09a2d0.md`](vcm-drive-impedance/records/20260825-162620-e09a2d0.md) (ideal, Z_vcm = 0); [`20260825-163251-cb36f0a.md`](vcm-drive-impedance/records/20260825-163251-cb36f0a.md) (at budget); [`20260825-163508-64203b5.md`](vcm-drive-impedance/records/20260825-163508-64203b5.md) (5× beyond); [`DR-0026`](../spec/decision-records/DR-0026-vcm-drive-source.md); `spec/testbench-suite-memo.md` §12 item 3 |
...
| Supply (V_DD ±10 %; C_dec, Z_vdd) | V_DD = 3.3 V ±10 % (2.97 / 3.30 / 3.63 V grid), single supply, 3.3 V devices throughout ([DR-0004](../spec/decision-records/DR-0004-device-flavor.md)); external decoupling **≥ 40 nF** at the V_DD pin; effective source impedance **≤ 3 Ω** in the switching band; **plus a required on-die local decoupling term inside `ADC_BLOCK`, not yet sized** ([DR-0036](../spec/decision-records/DR-0036-vdd-decoupling-budget.md), **proposed — requires operator sign-off, i.e. not yet ratified**, and carried in `README.md#target-specification` with that caveat inline). Stretch: `Z_vdd ≈ ≤ 1.6 Ω @ 2 MS/s` (conversion period halves; not further resolved, DR-0036) | **This row has two halves and they are verified differently — stated separately rather than averaged into one verdict.** **(1) The ±10 % supply window** is spanned by the supply axis of every PVT sweep cited above: 2.97 / 3.30 / 3.63 V on every grid in this table. **(2) The DR-0036 drive requirement is *derived*, not measured, and has no measured verdict.** Its input is measured — [`sim/adc-rail-current/`](adc-rail-current/records/20260923-002945-1cefe83.md) (27-point PVT grid) reports a **34.383980 mA** `i_vdd_peak_ua` at `ff_-40c_3.63v` (of which the CDAC bottom-plate driver branch carries 34.2987 mA, **99.75 %**, at the peak instant `t = 7.00286 µs`) against a **39.5421 µA** window average in that same corner, a factor of **869**, and a grid-worst average of **40.120 µA** at `ff_125c_3.63v`. DR-0036 bounds the per-conversion charge from that average (`ΔQ_conv ≤ 40.120 pC`), divides by the 0.5 LSB = 1.611328 mV allowance to get `C_dec ≥ 24.90 nF`, and provisions **≥ 40 nF** (per-conversion sag 1.003 mV) with `Z_vdd,max ≈ 3.28 Ω` → provisioned **≤ 3 Ω**. **What no record in this repo contains is a measurement of the converter behind such a source.** Every ADC-level deck in this table models the three supply islands as ideal DC sources (`sim/adc-power/testbench/tb_adc_power.spice`: `vddc`/`vddd`/`vddt` are `dc {vdd_val}`), so what the ≥ 40 nF / ≤ 3 Ω budget buys — and what missing it costs — is **unmeasured**. That is exactly the position the `V_CM` row was in before issue #358 measured it against DR-0026's budget; the V_DD counterpart campaign has not been run (**#393**). Two further bounds travel with the budget and are stated, not absorbed: the switching event's charge is **bounded from the window average, never integrated** (route A is 4.3× looser than the CDAC-branch attribution, and the event's width — ≤ 1.150 ns total / ≈ 0.272 ns attributed — is derived rather than measured, both below the deck's own 2 ns timestep), which **#386** is filed to close; and DR-0036 step 8's **523 Ω** landing-site-to-decode-bank effective resistance was read off [`layout/power/records/20260923-010407-d84c7b4.md`](../layout/power/records/20260923-010407-d84c7b4.md), **before** #378/PR #388 moved the block-level straps to Metal2 — the same `peak-upper-bound.COMPARATOR` case now reads **4.93 V** instead of 17.99 V in the governing [`20260923-084734-5f13cf9`](../layout/power/records/20260923-084734-5f13cf9.md) (`layout/power/README.md`), so the resistance behind clause 3's *placement* argument has fallen and DR-0036's own Consequences clause — *"whichever of #378 / this record is settled second must re-read the other"* — is **unsatisfied**: #388 landed second and did not re-read it. **(3) The on-die term is required and unmet as drawn.** `ADC_BLOCK` carries no on-die decoupling at all, and DR-0036 deliberately does not size it; see the "What is not yet measured" section below | **PASS on the ratified ±10 % supply window** (no dedicated record — it is the corner axis every other row already sweeps). **No measured verdict on DR-0036's decoupling / source-impedance budget, and none is available from what is committed**: it is a drive requirement on whoever integrates this block, derived from a measured rail current, and nothing in `sim/` has been run against a V_DD source that meets or misses it. **DR-0036 is *proposed*, not operator-ratified** ("Status: proposed — requires operator sign-off"), so the ≥ 40 nF / ≤ 3 Ω figures are reported here against a proposed budget — the same footing on which the `Area` row above reports DR-0024's proposed `< 0.16 mm²` — not graded as a ratified bound this row passes or fails. **The on-die `ADC_BLOCK` term is a stated, unmet requirement**, which DR-0036's own Consequences call *"a stated FAIL-shaped gap rather than an unasked question … not a formality this block passes"*; it is unsized, so it cannot be graded either, and it is carried in the "not yet measured" list rather than folded into the PASS above | [`spec/decision-records/DR-0036-vdd-decoupling-budget.md`](../spec/decision-records/DR-0036-vdd-decoupling-budget.md) (**proposed — requires operator sign-off**; the budget, its derivation and its four open questions); [`sim/adc-rail-current/records/20260923-002945-1cefe83.md`](adc-rail-current/records/20260923-002945-1cefe83.md) (27-point PVT grid — the 34.383980 mA peak, the 39.5421 / 40.120 µA averages and the per-branch split the budget is derived from; its own manifest states the peak is a **lower** bound, twice over); [`layout/power/records/20260923-010407-d84c7b4.md`](../layout/power/records/20260923-010407-d84c7b4.md) (the solved rail DR-0036 step 8 reads 523 Ω off) and [`layout/power/records/20260923-084734-5f13cf9.md`](../layout/power/records/20260923-084734-5f13cf9.md) (**current governing**, post-#378/#388 Metal2 straps); [`DR-0034`](../spec/decision-records/DR-0034-supply-droop-budget.md) (the *static* droop budget, also proposed, which DR-0036 does not re-open); [`DR-0004`](../spec/decision-records/DR-0004-device-flavor.md) (device flavour and the supply itself); every PVT record above (the ±10 % window). Filed follow-ups: **#386** (integrate the event directly), **#393** (drive the converter through a real V_DD network, the V_CM row's #358 done for this rail) |
...
- **On-die local decoupling inside `ADC_BLOCK` — required, unsized, and not
  drawn.** This is the `Supply` row's third clause, reported here rather than
  as a value in the row because it has no value to report.
  [DR-0036](../spec/decision-records/DR-0036-vdd-decoupling-budget.md) clause
  3 (**proposed — requires operator sign-off, not yet ratified**) requires a
  local decoupling capacitance drawn adjacent to the CDAC decode banks,
  *inside* this block — not at its boundary and not in the parent — because
  the off-die ≥ 40 nF cannot deliver the sub-nanosecond leading edge of the
  34.38 mA switching peak through a package loop inductance. **The block as
  drawn has none**, which DR-0036's own Consequences record as "a stated
  FAIL-shaped gap rather than an unasked question". It is **unsized on
  purpose**: `C_local ≥ L·I_pk²/(2·ΔV_inst²)` needs a package loop inductance
  (this repo has no package model) and a ratified instantaneous-excursion
  allowance `ΔV_inst` for `V_DD` (none exists — DR-0034's 33 mV is a *static*
  figure, already spent on the static drop), and the record's own sensitivity
  table spans **5.43 pF to 228 nF** — 0.0027 mm² to 114 mm² at 2 fF/µm² —
  across the plausible `ΔV_inst` range. Nothing in the `Supply` row can be
  graded against it until one of those two inputs exists. Two filed
  follow-ups bound it from different ends — **#386** (integrate the switching
  event directly, replacing the 4.3×-loose charge bound with a measured
  `ΔQ_event` and width) and **#393** (drive the converter through a real
  `V_DD` network at the budget, the `V_CM` row's #358 done for this rail) —
  and the geometry it lands on moved after DR-0036 was written: #378 / PR #388
  put the block-level straps on Metal2, which DR-0036's Consequences say
  obliges a mutual re-read that has not happened.
