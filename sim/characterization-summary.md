...
| V_CM (Z_vcm, C_dec) | V_cm = V_REF/2 = 1.65 V ext.; ≥ 40 nF decoupling; Z_vcm ≤ 220 Ω ([DR-0026](../spec/decision-records/DR-0026-vcm-drive-source.md), **proposed — not yet operator-ratified**) | **The budget is derived, not measured; what is measured is the converter's sensitivity to a real V_cm network.** Replacing the ideal, zero-impedance V_cm source every other ADC-level deck in this table uses with an R‖L + C_dec network (modelled exactly the way this same deck already models V_REF, DR-0002) moves `gain_err_lsb` **−2.005 → −2.205 → −2.312 LSB** at Z_vcm = 0 / 220 Ω / 1100 Ω — a real ≈ 0.2 LSB (≈ 10 %) shift **at the derived budget alone**, growing beyond it. Per-code errors move more and **not monotonically** (`inl_t256_lsb` −0.0134 → −0.2805 → −0.0980 LSB; `inl_t768_lsb` changes sign between the budget and 5× points), all staying well inside the ratified ±1 LSB INL window; `vref_droop_mv` is unaffected (0.383 → 0.382 → 0.376 mV), confirming the two rails' budgets are independent. **Corner subset, stated**: the `cdac` 7-corner capacitor-family process axis at 27 °C / 3.30 V only — an exploratory sensitivity sweep for a new decision record's derivation, **not** a full-PVT re-verification, justified in each record's own `Corner matrix run` block. The `gain_err_lsb` / `inl_t*_lsb` quantities are `sim/adc-inl-dnl/`'s own converter-level metrics and are explicitly **not** the ratified `Gain error, systematic` row | **No verdict against a ratified row is possible or claimed** — DR-0026 is *proposed*, pending operator ratification, and this is a sensitivity sweep rather than a spec check. What it does establish, and what every schematic row above should now be read against: **the ideal-V_cm-source assumption is not shown to be conservative** — it is a real, now-quantified gap, not merely a documented one. **No ratified row is shown to fail by it.** Re-running the three ADC-level decks against a real V_cm network at full PVT is a named, unclaimed follow-up (**#358**) | [`sim/vcm-drive-impedance/records/20260825-162620-e09a2d0.md`](vcm-drive-impedance/records/20260825-162620-e09a2d0.md) (ideal, Z_vcm = 0); [`20260825-163251-cb36f0a.md`](vcm-drive-impedance/records/20260825-163251-cb36f0a.md) (at the derived budget, 220 Ω / 40 nF); [`20260825-163508-64203b5.md`](vcm-drive-impedance/records/20260825-163508-64203b5.md) (5× beyond, 1100 Ω / 40 nF); [`DR-0026`](../spec/decision-records/DR-0026-vcm-drive-source.md) §"Evidence: is the ideal-source assumption conservative?" (the three-point table above, and the modelling caveat that the R‖L network is a lightly-damped resonant tank rather than a single-pole RC, which is what makes the per-code movement non-monotonic); `spec/testbench-suite-memo.md` §12 item 3 |
