...
- **The Gain error, mismatch resizing decision** — issue #177 /
  ...
  **Per-campaign status of that re-verification, as of this revision**
  (six schematic-level slices, one per campaign whose recorded result
  depends on `C_u` — #197's own scope named four, and re-running them found
  two more decks PR #202 had regenerated without re-taking their records).
