...
- **The resize closed one row and opened two.** DR-0019 (#177) resized
  ...
  **#211**. Power grows 13.4 % and still passes with 2.4× stretch margin;
  bit settling, the top-plate `C_par` divider and the `Gain error, systematic`
  row are unchanged or better. Static INL/DNL's re-take is in flight (#203).
- **Every post-layout (extracted) number in this repo now predates the
  resize.** No re-extraction of the resized layout exists yet (**#218**).
  Because the extracted result is the one this repo reports as *governing*
  where both exist, the SFDR row currently has no valid governing result at
  all: its extracted PASS describes an array that is no longer drawn.
...
| Full-ADC characterization (aggregated) | Current as of 2026-08-17, re-taken at DR-0019's resized `C_u` for five of the six `C_u`-dependent campaigns (#197 and sub-issues #204/#205; INL/DNL in flight, #203) — [`sim/characterization-summary.md`](sim/characterization-summary.md), one row per ratified spec line with a dated citation; the consolidated before/after adjudication of the resize is [`spec/testbench-suite-memo.md`](spec/testbench-suite-memo.md) §11.9. **Every extracted (post-layout) citation in it is pre-resize** — #218 |
...
#197, and five of its six schematic campaigns have now been re-run. **What
they found is not free**: the ENOB row newly fails at 2 of 9 corners and the
SFDR miss widens from 0.67 dB to 5.59 dB (#211), power grows 13.4 % and still
passes, settling does not move, and the `Gain error, systematic` row and the
top-plate divider *improve*. Static INL/DNL is in flight (#203) and the whole
post-layout side is still pre-resize (#218). Consolidated before/after with
...
PVT verification suite at the new `C_u` (issue #197, largely done at the
schematic level; #203 in flight, #218 for the post-layout half) and reconciling
the resize's area growth against DR-0017's budget (issue #198) — is tracked
under issue #190.
