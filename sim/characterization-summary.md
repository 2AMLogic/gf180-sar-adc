...
| Gain error, systematic | ≤ 0.5 LSB, untrimmed, excluding V_REF error | **Re-run at the physically-implemented DR-0019 resize (`C_u = 35.6528 fF`, issue #197): schematic 0.0021–0.0039 LSB** (was 0.0045–0.0088 LSB pre-resize) — the row *improves* 2.26×, to ~127× inside the bound, because `C_par` does not scale with `C_u` while `C_arr` does. The **0.00046–0.0016 LSB extracted** figure is now **STALE**: taken on the pre-resize layout (issue #131) and not re-measured since #196/#202 (issue #218) | **PASS** (schematic, current design; wider margin than before the resize) | [`sim/dr0014-sampling/records/20260817-134517-cde979d.md`](dr0014-sampling/records/20260817-134517-cde979d.md) (schematic, resized `C_u`, issue #197, supersedes `20260802-141402-1224e11`); [`sim/dr0014-sampling/records/20260807-091733-434dc37.md`](dr0014-sampling/records/20260807-091733-434dc37.md) (extracted, **stale**, pre-resize, issue #131); `spec/testbench-suite-memo.md` §11.9.6; `sim/extracted-delta-summary.md` §4.9 |
...
- **The Gain error, mismatch resizing decision** — issue #177 /
  ...
  **Per-campaign status of that re-verification, as of this revision**
  ...
  - **Extracted (post-layout) re-verification of all of the above — still
    open, filed as issue #218.** Every extracted figure this document cites
    was measured against the pre-resize layout; that is flagged at the top of
    `sim/extracted-delta-summary.md` and, for the consequence it has on the
    SFDR row's *governing* verdict specifically, in
    `spec/testbench-suite-memo.md` §11.9.8.
