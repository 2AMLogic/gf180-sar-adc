...
layout/adc-top/
  ...
  sar_ctrl/                the SAR-logic sequencer's placed-and-routed macro
                           (issue #274) -- see sar_ctrl/README.md; driven by
                           design/sar-logic/flow/pnr_sar_ctrl.py, not by a
                           generator in this directory (its "source" is the
                           synthesized netlist + `klt place-and-route`, not
                           hand-placed KLayout primitives)
```
...
**Update (issue #274, SAR-logic sequencer place-and-route): the "SAR-logic
reserved region incl. its ring" row in the table above (7,624 µm², an
issue #118-era snapshot) is stale.** The same #215 strap-corridor
re-derivation above grows the analog core, and `gen_adc_top.py`'s digital
footprint tracks it — `digital_box`'s own width is
`max(SAR_RESERVED_W, analog_ring.width() // 3)`, and the wider analog ring
now drives that `// 3` term past the nominal `SAR_RESERVED_W` floor. The
currently committed `area.json` (regenerated byte-identical against
`origin/main` at this update's own commit) reports
`areas_um2.sar_logic_reserved` = **8,646.028 µm²** (ring-inclusive), i.e. a
ring-exclusive core box of 199.21 × 40.0 µm = **7,968.4 µm²** — not
7,624 µm². Nothing downstream may quote 7,624 µm² as the reserved
SAR-logic footprint any more; that table cell is left as drawn, per this
file's own "supersede rather than edit in place" convention for the "Area,
as drawn" table.

Issue #274 (DR-0023 follow-on (b)) placed and routed #272's synthesized
`sar_ctrl_a` gate netlist (`gf180mcu_fd_sc_mcu7t5v0`) via
`klt place-and-route` inside an EXPLICIT floorplan sized to exactly match
this current 199.21 × 40.0 µm core box (0 margin either dimension) — see
`design/sar-logic/flow/pnr_sar_ctrl.py` and its own evidence record under
`design/sar-logic/flow/sar_ctrl/records/`. **It fits**: `target_stage:
"route"` completed (`status: "ok"`), `route_drc_violation_count: 0`,
`antenna_violation_count: 0`, `klt drc --deck gf180mcu` clean (0
violations) against the merged, routed GDS at `utilization_pct` 67.72 % of
the reserved core. The routed layout artifacts (merged GDS, DEF, as-built
Verilog) are committed at `layout/adc-top/sar_ctrl/sar_ctrl.{gds,def,v}` —
a **separate** macro alongside `adc_top.gds`/`adc_block.gds`, not merged
into either's own top cell (hard-macro composition into the analog block's
GDS is not this issue's scope). No SDC/STA claim is made (follow-on (c),
issue #275, a separate filed issue) — see that record's own "No SDC/STA
claim" section.
