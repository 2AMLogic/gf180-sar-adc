...
| Verification suite | Schematic-level, 9-corner ADC suite: INL/DNL and ENOB pass; SFDR misses the ≥ 62 dB target by 0.67 dB at one corner of nine — `spec/testbench-suite-memo.md` §11.2. Post-layout extracted re-run **partially done** (#89): static linearity is re-run against the parasitic-extracted CDAC core over 27 PVT corners and still passes (INL −0.111 LSB, DNL 0.100 LSB worst; deltas ≤ 0.003 LSB, no corner changes verdict). ENOB/SFDR, power and the extracted-core Monte Carlo question are not yet done — `sim/extracted-delta-summary.md` |
| Silicon | None |
